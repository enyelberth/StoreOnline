...
<nav class="bg-white border-gray-200 ">
  ...
  <a href="#" class="flex items-center space-x-3 rtl:space-x-reverse">
      ...
      <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">StoreOnline</span>
  </a>
  <div class="flex ">
    ...
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
      ...
      <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white ">
        <li v-for="item in menuItems" :key="item.id">
          <nuxt-link :to="item.link" href="#" class="block py-2 px-3 text-black bg-gray-700 rounded md:bg-transparent  md:p-0 " aria-current="page">{{ item.name }}</nuxt-link>
        </li>
